...
const TaskForm = () => {
  ...
  const handleSelectChange = async (name, option) => {
    ...
    if (name === "project_id") {
      setTask((prev) => ({
        ...prev,
        [name]: value,
        assigned_to: null, // Clear previous assignment when project changes
      }));
      ...
      if (value) {
        try {
          ...
          // Don't remap the data since it's already formatted by the server
        ...
        setProjectEmployees([]); // Clear employees if no project selected
      }
    } else {
      setTask((prev) => ({
        ...prev,
  ...
  useEffect(() => {
    ...
    console.log("Available project employees:", projectEmployees);
  }, [task, projectEmployees]);
